...
class ToRouteStreams {
   ...
   */
	Stream operator [](String eventType) => _bubbled ? _provider[eventType].bubbleStream : _provider[eventType].stream;
}
...
class ToDiscriminateStreams<T extends DiscriminatedEvent> {
   ...
   */
	DiscriminatorStreams<T> operator [](String discriminatedEventType) => this._bubbled ? (_provider[discriminatedEventType] as ToDiscriminateEventStreamProvider).onBubbleDiscriminatorEvents : (_provider[discriminatedEventType] as ToDiscriminateEventStreamProvider).onDiscriminatorEvents;
}
...
class DiscriminatorStreams<T extends DiscriminatedEvent> {
	...
	Stream<T> get stream => _bubbled ? _provider.bubbleStream : _provider.stream;

	/**
   *  Returns the stream for the given [discriminator] or
   *  null if [discriminator] is not managed.
   */
	Stream<T> operator [](String discriminator) => _bubbled ? _provider[discriminator].bubbleStream : _provider[discriminator].stream;
}
...
class BubbleProviderReference<T extends FLEvent> {
	...
	bool operator ==(other) {
		return bubblingId == other.bubblingId && identical(bubbleProvider, other.bubbleProvider);
	}
...
class BubbleTargetReference<T extends FLEvent> {
	...
	bool operator ==(other) {
		return bubblingId == other.bubblingId && identical(bubbleTarget, other.bubbleTarget);
	}
...
abstract class StreamProvider<T extends FLEvent> {
	...
	final StreamController<T> _controller = new StreamController.broadcast(sync: true);
	...
	StreamProvider([FLEventTarget target])
			: this._target = target,
			  this._hostProvider = null;

	StreamProvider._forHostProvider(this._hostProvider);

	FLEventTarget get target => _hostProvider != null ? _hostProvider.target : _target;

	void set target(FLEventTarget target) {
		if (_hostProvider != null) {
			throw new UnsupportedError("Setting target " + "on hosted provider not allowed");
	...
	void addBubbleTargetProvider(dynamic bubblingId, StreamProvider<T> bubbleProvider) {
		...
		if (_hostProvider != null) {
			throw new UnsupportedError("Adding bubble target provider to hosted" + " provider not allowed");
	...
	void removeBubbleTargetProvider(dynamic bubblingId, StreamProvider<T> bubbleProvider) {
		var reference = new BubbleProviderReference(bubblingId, bubbleProvider);
		if (_hostProvider != null) {
			throw new UnsupportedError("Removing bubble target provider from hosted" + " provider not allowed");
	...
	void _dispatchBubbleEvent(FLEvent event, StreamProvider<T> _targetProvider) {
		...
		if (_bubbleProviders != null) {
			_bubbleProviders.forEach((BubbleProviderReference reference) {
				...
				if (!event._contains(targetReference)) {
					var targetBubbleProvider = _targetProvider._getBubbleProvider(reference.bubbleProvider);

					FLEvent clonedEvent = event.clone();
					if (clonedEvent.runtimeType != event.runtimeType) {
						throw new UnimplementedError("Implement clone method " + "${event.runtimeType}.clone()");
					}
...
class ToRouteEventStreamProvider extends StreamProvider {
	...
	final ToDiscriminateStreams onBubbleToDiscriminateEvents = new ToDiscriminateStreams._alsoBubbled();

	final ToDiscriminateStreams onToDiscriminateEvents = new ToDiscriminateStreams._notBubbled();
	...
	FLEventStreamProvider operator [](String key) {
		...
		if (indexOf != -1) {
			...
			ToDiscriminateEventStreamProvider provider = _providers.putIfAbsent(eventType, () => new ToDiscriminateEventStreamProvider._forHostProvider(eventType, this));
			return discriminator.isEmpty ? provider : provider[discriminator];
		} else {
			return _providers.putIfAbsent(key, () => new FLEventStreamProvider._forHostProvider(key, this));
		}
	}

	StreamProvider _getBubbleProvider(StreamProvider provider) => provider;
}

class FLEventStreamProvider<T extends FLEvent> extends StreamProvider<T> {
	final String _eventType;

	FLEventStreamProvider(this._eventType, [FLEventTarget target]) : super(target);

	FLEventStreamProvider._forHostProvider(this._eventType, StreamProvider hostProvider) : super._forHostProvider(hostProvider);
	...
	StreamProvider _getBubbleProvider(StreamProvider provider) {
		if (_hostProvider != null) {
			return (_hostProvider._getBubbleProvider(provider) as ToRouteEventStreamProvider)[_eventType];
		} else {
			return provider;
		}
	}
...
class ToDiscriminateEventStreamProvider<T extends DiscriminatedEvent> extends FLEventStreamProvider<T> {

	final Map<String, DiscriminatedEventStreamProvider<T>> _providers = {};

	final DiscriminatorStreams<T> onBubbleDiscriminatorEvents = new DiscriminatorStreams._alsoBubbled();

	final DiscriminatorStreams<T> onDiscriminatorEvents = new DiscriminatorStreams._notBubbled();

	ToDiscriminateEventStreamProvider(String eventType, [FLEventTarget target]) : super(eventType, target) {
		...
		if (!eventType.endsWith(DiscriminatedEvent.DISCRIMINATOR_POSTFIX)) {
			throw new ArgumentError("Discriminated event type '" + eventType + "' must end with '" + DiscriminatedEvent.DISCRIMINATOR_POSTFIX + "' postfix like '" + (eventType + DiscriminatedEvent.DISCRIMINATOR_POSTFIX) + "'");
		}
	}

	ToDiscriminateEventStreamProvider._forHostProvider(String eventType, ToRouteEventStreamProvider hostProvider) : super._forHostProvider(eventType, hostProvider) {

		onBubbleDiscriminatorEvents._provider = this;
		onDiscriminatorEvents._provider = this;
	}

	DiscriminatedEventStreamProvider<T> operator [](dynamic discriminator) => _providers.putIfAbsent(discriminator, () => new DiscriminatedEventStreamProvider._forHostProvider(_eventType, discriminator, this));

	void dispatch(T event, [void onPreDispatching(T event), void onPostDispatched(T event)]) {
		throw new UnsupportedError("Dispatch event from " + "host provider not allowed");
	}
}

class DiscriminatedEventStreamProvider<T extends DiscriminatedEvent> extends FLEventStreamProvider<T> {

	final dynamic _discriminator;

	DiscriminatedEventStreamProvider(String eventType, this._discriminator, [FLEventTarget target]) : super(eventType, target) {
	...
	DiscriminatedEventStreamProvider._forHostProvider(String eventType, this._discriminator, ToDiscriminateEventStreamProvider<T> hostProvider) : super._forHostProvider(eventType, hostProvider);

	StreamProvider _getBubbleProvider(StreamProvider provider) {
		if (_hostProvider != null) {
			return (_hostProvider._getBubbleProvider(provider) as ToDiscriminateEventStreamProvider<T>)[_discriminator];
		} else {
			return provider;
		}
	}
